  ...
  return (
    <header className="fixed top-0 left-0 w-full bg-white/90 backdrop-blur-sm z-50 border-b border-white/20">
      <nav role="navigation" aria-label="Main navigation" className="max-w-6xl mx-auto flex items-center justify-between p-3 sm:p-4 bg-sky-950">
        <a href="/" className="text-lg sm:text-xl font-semibold tracking-wider" aria-label="EUROTRIP25 home page">
          EUROTRIP25
        </a>
        
        {/* Mobile menu button */}
        <button
          className="sm:hidden text-white hover:text-amber-600 transition-colors duration-200"
        ...
        {/* Desktop navigation */}
        <ul className="hidden sm:flex space-x-4 lg:space-x-6 text-xs sm:text-sm uppercase tracking-wide">
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-sky-950/95 backdrop-blur-sm sm:hidden">
            <ul className="flex flex-col space-y-2 p-4 text-sm uppercase tracking-wide">
              {["Itinerary", "Concierge", "Dates", "Contact"].map(label => (
                <li key={label}>
                  <button 
                    onClick={() => handleNavClick(label)} 
                    className="w-full text-left hover:text-amber-600 transition-colors duration-200 px-2 py-3 border-b border-white/10 last:border-b-0" 
                    aria-label={`Navigate to ${label} section`}
                  >
                    {label}
                  </button>
