      ...
      <CardContent className="p-0">
        <div className="p-4 bg-slate-700">
          <div className="flex justify-between items-center">
            ...
            {onViewMap && day.accommodation && day.accommodation.address && <Button variant="outline" size="sm" onClick={() => onViewMap(day)} className="flex items-center gap-2">
                <Map className="h-4 w-4" />
                View Map
              </Button>}
            {onEditDay && <Button variant="outline" size="sm" onClick={() => onEditDay(day)} className="ml-2">
                Edit Day
              </Button>}
          </div>
          ...
          <div className="mt-3">
            <DayMemento dayNumber={day.dayNumber} mementoImage={dayMementos[day.dayNumber] || null} onMementoChange={handleMementoChange} />
          </div>
        </div>
        
        <div className="p-4 bg-slate-800">
          {day.encouragement && <div className="mb-4 p-3 bg-blue-50 border-l-4 border-blue-500 rounded">
              <p className="text-sm italic text-blue-800">{day.encouragement}</p>
            </div>}
          
          {day.activities && day.activities.length > 0 ? <div className="mb-4">
              <h4 className="font-medium mb-2 text-gray-200">Activities</h4>
              <ul className="space-y-1">
                {day.activities.map((activity, i) => <li key={i} className="text-sm flex justify-between items-center group">
                    <div>
            ...
            </div> : <p className="text-sm text-gray-500 mb-4">No activities planned for this day.</p>}
          
          <Separator className="my-3" />
          
          {day.accommodation ? <div>
              <h4 className="font-medium mb-2">Accommodation</h4>
              <p className="font-medium text-sm">{day.accommodation.name}</p>
              {day.accommodation.address && <p className="text-sm text-gray-600">{day.accommodation.address}</p>}
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-sm">
                {day.accommodation.checkin && <p><span className="text-gray-500">Check-in:</span> {day.accommodation.checkin}</p>}
                {day.accommodation.checkout && <p><span className="text-gray-500">Check-out:</span> {day.accommodation.checkout}</p>}
                {day.accommodation.wifi && <p><span className="text-gray-500">WiFi:</span> {day.accommodation.wifi}</p>}
                {day.accommodation.contactPhone && <p><span className="text-gray-500">Contact:</span> {day.accommodation.contactPhone}</p>}
              </div>
            </div> : <p className="text-sm text-gray-500">No accommodation details available.</p>}
        </div>
      </CardContent>
    </Card>;
};