...
serve(async (req) => {
  ...
  try {
    const { messages, model, systemPrompt }: TravelBuddyRequest = await req.json();
    ...
    if (!response.ok) {
      ...
      throw new Error('Travel buddy service unavailable');
    }
    ...
    console.error('Travel buddy chat error:', error);
    return new Response(
